...
const TaskMarketPlace = () => {
  const { isAuthenticated, getTasksByPromisor, ProposetoupdateCompletionStatus, fetchUserProfile, whoamiActor, principal } = useAuth(); 
  ...
  const fetchTasks = async () => {
    try {
      const fetchedTasks = await getTasksByPromisor();
  ...
  const proposeToComplete = (taskId) => {
    ...
    ProposetoupdateCompletionStatus(natTaskId)
      .then((response) => {
        if (response.success) {
          console.log(`Successfully Completed task: ${natTaskId}`);
          alert('Task completed successfully!');
        } else {
          console.error(`Failed to complete task with ID: ${natTaskId}`, response.error);
          alert('Failed to submit the task completion. Please try again.');  // Show an error message if the proposal fails
        }
      })
      .catch((error) => {
        console.error(`Error completing task with ID: ${natTaskId}`, error);
        alert('An error occurred while submitting the completion.');  // Handle any network or unexpected errors
      });
  };
        ...
        <div className="text-center mb-6">
          ...
          {!loading && tasks.length === 0 && (
            <p className="text-black flex items-center justify-center">
              <FaTasks className="mr-2" /> You have not been approved for any tasks.
            </p>
          )}
        </div>
